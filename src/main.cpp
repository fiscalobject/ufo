...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int DiffMode = 1;
    if ((pindexLast->nHeight >= nHardForkTwo) || (fTestNet && (pindexLast->nHeight >= 10))) {
        DiffMode = 2;
    }
    
    /* The 3rd hard fork and testnet hard fork */
    if((nHeight >= nHardForkThree) || (fTestNet && (nHeight >= nTestnetFork))) {
        ...
        if((nHeight <= nHardForkThree + 10)  || (fTestNet && (nHeight <= nTestnetFork + 10)))
          DiffMode = 1;
    }
...
void static UFOMiner(CWallet *pwallet)
{
    printf("UFOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ufo-miner");
    ...
    try { loop {
        ...
        printf("Running UFOMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UFOMiner, pwallet));
}
