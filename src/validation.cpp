...
const std::string strMessageMagic = "UFO Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s UFO additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // New inflation model to prevent going over total supply of 4 billion
    if (nHeight >= 2000000) {
        // Remaining supply 93,768,738. Mint half every four years
        nSubsidy = 33 * COIN;
        halvings = (nHeight - 2000000 + 1) / 1401600;
        nSubsidy >>= halvings;
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // BIP34 is always active after block 146557 though there are still version 1 blocks after
    // that height, the last version 1 block is at height 160053 making BIP34v2Height block height 160054.
    if((block.nVersion < 2 && nHeight >= consensusParams.BIP34v2Height) ||
